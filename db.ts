...
export async function createBooking(booking: InsertBooking): Promise<Booking | null> {
  ...
  try {
    ...
    const created = await db.select().from(bookings).where(eq(bookings.id, insertedId as number)).limit(1);
...
export async function updateBooking(id: number, updates: Partial<InsertBooking>): Promise<Booking | null> {
  const db = await getDb();
  if (!db) {
